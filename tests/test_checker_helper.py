...
class TestHelper(unittest.TestCase):
    ...
    def test_process_certmonger_data(self):
        raw_data = """
...
Request ID '20170331122405':
    ...
    auto-renew: yes
        ...
        expected = [
            {'status': 'MONITORING', 'auto-renew': 'yes', 'pre-save command': '/usr/libexec/ipa/certmonger/stop_pkicad', 'certificate': 'type=NSSDB,location=/etc/pki/pki-tomcat/alias,nickname=auditSigningCert cert-pki-ca,token=NSS Certificate DB', 'track': 'yes', 'CA': 'dogtag-ipa-ca-renew-agent', 'expires': '2019-03-21 132314 UTC', 'Request ID': '20170331122404', 'stuck': 'no', 'key pair storage': 'type=NSSDB,location=/etc/pki/pki-tomcat/alias,nickname=auditSigningCert cert-pki-ca,token=NSS Certificate DB,pin set', 'key usage': 'digitalSignature,nonRepudiation', 'post-save command': '/usr/libexec/ipa/certmonger/renew_ca_cert "auditSigningCert cert-pki-ca"', 'subject': 'CN=CA Audit,O=DOM-058-188.ABC.IDM.LAB.ENG.BRQ.REDHAT.COM', 'issuer': 'CN=Certificate Authority,O=DOM-058-188.ABC.IDM.LAB.ENG.BRQ.REDHAT.COM'},
            {'status': 'MONITORING', 'auto-renew': 'yes', 'pre-save command': '/usr/libexec/ipa/certmonger/stop_pkicad', 'certificate': 'type=NSSDB,location=/etc/pki/pki-tomcat/alias,nickname=ocspSigningCert cert-pki-ca,token=NSS Certificate DB', 'track': 'yes', 'CA': 'dogtag-ipa-ca-renew-agent', 'expires': '2019-03-21 132313 UTC', 'Request ID': '20170331122405', 'stuck': 'no', 'key pair storage': 'type=NSSDB,location=/etc/pki/pki-tomcat/alias,nickname=ocspSigningCert cert-pki-ca,token=NSS Certificate DB,pin set', 'key usage': 'digitalSignature,nonRepudiation,keyCertSign,cRLSign', 'eku': 'id-kp-OCSPSigning', 'subject': 'CN=OCSP Subsystem,O=DOM-058-188.ABC.IDM.LAB.ENG.BRQ.REDHAT.COM', 'post-save command': '/usr/libexec/ipa/certmonger/renew_ca_cert "ocspSigningCert cert-pki-ca"', 'issuer': 'CN=Certificate Authority,O=DOM-058-188.ABC.IDM.LAB.ENG.BRQ.REDHAT.COM'}]

        self.assertEqual(expected, helper.process_certmonger_data(raw_data))
